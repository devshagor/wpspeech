<?php
/**
 * Frontend player for WP Text to Speech.
 *
...
class WP_TTS_Frontend {
	...
	public function __construct() {
		$this->options = get_option( WP_TTS_OPTION_KEY, array() );
	...
	public function maybe_enqueue_assets() {
		...
		wp_enqueue_style(
			'wp-tts-frontend',
			WP_TTS_PLUGIN_URL . 'assets/css/wp-tts-frontend.css',
			array(),
			WP_TTS_VERSION
		);
		...
		wp_enqueue_script(
			'wp-tts-frontend',
			WP_TTS_PLUGIN_URL . 'assets/js/wp-tts-frontend.js',
			array(),
			WP_TTS_VERSION,
			true
		);

		wp_localize_script( 'wp-tts-frontend', 'wpTtsSettings', array(
			...
			'i18n'             => array(
				'listen'      => __( 'Listen', 'wp-text-to-speech' ),
				'pause'       => __( 'Pause', 'wp-text-to-speech' ),
				'resume'      => __( 'Resume', 'wp-text-to-speech' ),
				'unsupported' => __( 'Text-to-speech is not supported in this browser.', 'wp-text-to-speech' ),
			),
		) );
	}
	...
	public function inject_player( $content ) {
		...
		if ( has_block( 'wp-tts/player', get_the_ID() ) ) {
			return $content;
		}
	...
	private function has_player_block() {
		...
		return has_block( 'wp-tts/player', get_queried_object_id() );
	}
}
